...
class MyLineChart {
  ...
  final DateFormat _formatDayMonth = DateFormat('dd MMM');
  ...
  MyLineChart(this.lines, this.fromTo);

  factory MyLineChart.fromDateTimeMaps(List<Map<DateTime, double>> series,
    ...
    return MyLineChart(convertFromDateMaps.left, convertFromDateMaps.right);
  }
  ...
  void initialize(double widthPX, double heightPX) {
    ...
    for (int axisIndex = 0; axisIndex < indexToUnit.length; axisIndex++) {
      ...
      for (int c = 0; c <= (stepCount + 1); c++) {
        int axisValue = (_minY[unit] + _yTicks[unit] * c).toInt();
        ...
        TextSpan span = new TextSpan(
            style: new TextStyle(
                ...
                fontSize: 12),
            text: double.parse(axisValueString).toInt().toString());
