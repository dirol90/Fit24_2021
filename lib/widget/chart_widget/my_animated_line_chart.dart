...
import 'my_area_line_chart.dart';
import 'my_line_chart.dart';

class MyAnimatedLineChart extends StatefulWidget {
  final MyLineChart chart;

  const MyAnimatedLineChart(
      this.chart, {
        Key key,
      }) : super(key: key);

  @override
  _MyAnimatedLineChartState createState() => _MyAnimatedLineChartState();
}

class _MyAnimatedLineChartState extends State<MyAnimatedLineChart>
...
class ChartPainter extends CustomPainter {
  ...
  final DateFormat _formatMonthDayHoursMinutes = DateFormat('dd MMM');
  ...
  void _drawAxisValues(Canvas canvas, Size size) {
    ...
    for (int c = 0; c <= (_stepCount + 1); c++) {
      _drawRotatedText(
          canvas,
//          _chart.xAxisTexts[c],_chart.axisOffSetWithPadding + (c * _chart.widthStepSize) - 15 ,size.height - (LineChart.axisOffsetPX + 20),0
          _chart.xAxisTexts[c],
  ...
  void _drawLines(Size size, Canvas canvas) {
    ...
    _chart.lines.forEach((chartLine) {
      ...
      if (_chart is MyAreaLineChart) {
        MyAreaLineChart areaLineChart = _chart;
